...
object Main extends App {
  ...
  override def main(args: Array[String]) {
    val spark = SparkSession
      .builder()
      .appName("CSE511-HotspotAnalysis-ZhandauletYespossynov") // YOU NEED TO CHANGE YOUR NAME
  ...
  private def queryLoader(spark: SparkSession, queryName: String, queryParams: String, outputPath: String) {
    ...
    if (queryName.equalsIgnoreCase("hotcellanalysis")) {
      if (queryParam.length != 1) throw new ArrayIndexOutOfBoundsException("[CSE511] Query " + queryName + " needs 1 parameters but you entered " + queryParam.length)
